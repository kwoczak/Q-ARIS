...
import { updateMuseumAction } from "@/app/actions/admin"
import { UserCog } from "lucide-react"
import { validatePassword } from "@/lib/password-utils"

export function EditMuseumDialog({ museum }: { museum: any }) {
    ...
    async function onSubmit(formData: FormData) {
        ...
        try {
            const res = await updateMuseumAction(formData)
            if (res?.error) {
                setError(res.error)
            } else {
                setOpen(false)
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>Edit Museum Account</DialogTitle>
                    <DialogDescription>
                ...
                <form action={onSubmit}>
                    <div className="flex flex-col gap-4 py-4">
                        <div className="flex flex-col gap-2">
                            ...
                            <Input id="username" name="username" defaultValue={museum.username} required />
                        </div>
